...
const NewChat = () => {
  ...
  const [recipientLoading, setRecipientLoading] = useState(
    !location.state?.user
  );
  ...
  const chatOpen =
    location.pathname !== "/chats" || location.path !== "/groups";
  ...
  useEffect(() => {
    ...
    const existConversation = conversations?.find(
      (conv) =>
        ...
        ) // conversation have the choosen user
    );

    if (existConversation) {
      // if exist, navigate to chat page to continue chat
  ...
  const onSendMessage = useCallback(() => {
    ...
    createConversation(
      ...
      (response) => {
        ...
        // Send message immediately via socket (don't wait for navigation)
        sendMessage({
          type: "TEXT",
  ...
  return (
    ...
    <div
      className={`conversation conversation--direct ${chatOpen ? "conversation--open" : ""}`}
    >
      <div className="conversation__body">
        ...
        <form className="conversation__form">
          ...
          <div className="icon-container send-message" onClick={onSendMessage}>
            <SendHorizonal />
          </div>
        </form>
      </div>
